...
In the case where the input is described by classical multimode coherent states $\ket{{\alpha}} = \ket{\alpha_1, \ldots, \alpha_n}$, the output is a coherent state $\ket{\beta}$ as well due to linearity and
\[
  \beta_j = \sum_k M_{j,k} \,\alpha_k
  \label{eq:pl.coherent_transfer_matrix}
\]
holds.
Note that throughout this chapter, we have to distinguish between the coherent state $\ket{\alpha}$ and the corresponding coherent state vector $\alpha$.
On the one hand, due to describing a pure quantum state, the former is always normalized such that $\Norm{\ket\alpha}^2 = \Braket{\alpha, \alpha} = 1$.
On the other hand, the normalization of a coherent state vector $\alpha$ describes the intensity of the classical light.
...
In this chapter, we propose an approach for recovering $M$ using random input coherent states $\alpha$ sampled from appropriate distributions.
...
As shown in \cref{fig:experimental.schematic}, we are able to prepare any input state from a single laser input in the bottom mode using the left-most cascade of couplers and phase-shifters.
Since varying the norm of the prepared input vector $\alpha$ requires to change the intensity of the input or \quotes{dumping} parts of the input in the topmost mode, it is beneficial to consider ensembles with fixed intensity in the following.
...
However, the properties of the components, and therefore, also the transfer matrix are generally dependent on the wavelength, polarization, etc.\ of the light.
Therefore, a characterization using coherent light is unsuitable in general for predicting the performance of the device when used with single photon sources.
Instead, the device should be evaluated under the same experimental conditions where it will be used.
For this purpose, we estimate the outcomes of the intensity measurements~\eqref{eq:pl.intensities} using single photon states.

When a single photon Fock state is input in the bottom waveguide, the prepared state prior to $M$ is
\[
  \ket{\psi({\alpha})} = \sum_j \alpha_j a_j^{\dag} \ket{\mathfrak{0}},
\]
where $\ket{\mathfrak{0}}$ denotes the vacuum state and $a_j^\dagger$ the creation operator of a photon in the $j$-th mode.
...
To be more specific, if we subsequentially feed $N$ single photon Fock states into the device such that they do not interfer, the photon counting statistics is governed by a multinomial distribution, i.e.
\[
...
Therefore, if we take into account this additional cost for reconfiguring the preparation, reconstructions with a smaller number of more precise measurements perform better when the \quotes{time budget} is kept fixed.
