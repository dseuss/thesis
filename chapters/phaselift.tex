...
In the case where the input is described by a classical multi-mode coherent state $\ket{{\alpha}} = \ket{\alpha_1, \ldots, \alpha_n}$, due to linearity, the output is a coherent state $\ket{\beta}$ as well.
Its components are given by
\[
  \beta_j = \sum_k M_{j,k} \,\alpha_k.
  \label{eq:pl.coherent_transfer_matrix}
\]
Note that throughout this chapter, we have to distinguish between coherent state vector $\alpha \in \Complex^n$ and the corresponding coherent state $\ket{\alpha}$ defined in terms of Fock basis vectors $\ket{k}$
\[
...
\]
On the one hand, due to describing a pure quantum state, the coherent state is always normalized such that $\Norm{\ket\alpha}^2 = \Braket{\alpha | \alpha} = 1$.
...
However, $x$ can only be recovered up to a global phase since $x$ and $\ee^{\ii\phi} x$ are indistinguishable from the measurements~\eqref{eq:pl.phase_retrieval_measurements} for any phase angle $\phi$.
...
In case of the Gaussian ensemble, the components $\alpha\ind{l}_i$ of the measurement vectors are i.i.d.\ standard complex Gaussian random variable.
...
\begin{proposition}%
  ...
  \begin{enumerate}
    \item \emph{Gaussian sampling scheme:} $\alpha \in \mathbb{C}^n$ chosen from the standard complex normal distribution $\mathcal{N}(0,\tfrac{1}{2}\mathbb{\1_n})+ i \mathcal{N}(0,\tfrac{1}{2}\mathbb{\1_n})$.
    In this case
    \[
      C_\mathrm{I} = C_\mathrm{SI} = C_\mathrm{SG} = 1.
    \]

    \item \emph{Uniform sampling scheme:} $\alpha \in \mathbb{C}^n$ chosen uniformly from the complex sphere with radius $\sqrt{n}$.
    In this case
    \[
...
The following fundamental theorem provides rigorous recovery guarantees for phase retrieval via PhaseLift.
...
We postpone the proof of this result to \cref{sec:pl.main_proof} in order to derive an error bound for the recovered signal vector $x^\sharp$ from \cref{thm:pl.phaselift_noisy}.
...
In \cref{sub:pl.recr}, we introduced different multiple ensembles to choose the $\alpha\ind{l}$ from.
...
\begin{figure*}[tbp]
  ...
  \todo[inline]{Explain this picture in the text}
  }
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\todo{Some more interpretation of these results would be nice}
