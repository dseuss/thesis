...
Note that although we are recovering $\ketbra{x}$, which is embedded in the $n^2$ dimensional space of all $n \times n$ matrices, the demand on the number of measurements $m$ in \cref{thm:phaselift_noisy} scales linearly in the original problem's dimension $n$.
...
\begin{protocol}%
  ...
  In order to recover a transfer matrix $M$ experimentally from measured intensities~\eqref{eq:pl.intensities_as_overlap},
  \begin{enumerate}
...
The rest of this section is devoted to adapting the results from \cref{sub:pl.recr} to derive rigorous performance guarantees for \cref{prot:characterization}.
