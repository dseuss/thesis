...
%Note that whenever our discussion is not conceptually restricted to $d$-dimensional quantum states (what happens in the current subsection), we omit the subscript $d$.
%The set  $\HermTrace$ can be as well understood as a general parameter space with $\States$ playing the role of its subset obtained by imposing certain constraints (such as positivity).
%Consequently, $\varrho_0$ can be understood in a more abstract way as a parameter belonging to $\HermTrace$.
...
% \todods{Take out this whole paragraph?}
% Another possible notion of optimality, the minimax principle, favors confidence regions that have the smallest expectedvolume for the worst choice of the true parameter~\cite{Kiefer_2012_Introduction}.
% However, since minimax estimators try to reduce the worst case volume at all costs, they often show a very singular and counter-intuitive behavior -- even for ``regular values'' of the unknown parameter.\\


Def.~\ref{def:ortho.admissible} can also be stated for the parameter estimation with physical constraints, i.e.\ when $\varrho_0 \in \States$.
...
% For the answer, define the following region estimators: $\CR^\cap := \CR\cap\States$, which is the part of $\CR$ belonging to $\States$, and $\CR^{c}=\CR\setminus \CR^{\cap}$, being the compliment of $\CR^{\cap}$ in $\CR$.
% Obviously $\CR = \CR^{\cap} \cup \CR^{c}$.
% We shall start our analysis by considering a list of generic scenarios for a given observation $\vec y$:
% \begin{enumerate}
%   \item\label{it:ortho.truncation} truncation scenario: the positivity constraint acts only on the boundary of $\States$, provided the boundary is reached. Then $\CR^+(\vec y) = \CR^\cap(\vec y)$, and consequently $\Vol(\CR^+(\vec y)) = \Vol(\CR^\cap(\vec y))$.
%   \item shrinkage scenario: $\CR^+(\vec y) \subset \CR^\cap(\vec y)$ and, therefore, $\Vol(\CR^+(\vec y)) < \Vol(\CR^\cap(\vec y))$.
%   \item shift \& deformation scenario: any other case in which $\CR^+(\vec y)$ covers some density matrices beyond $\CR^\cap(\vec y)$, and at the same time $\Vol(\CR^+(\vec y)) < \Vol(\CR^\cap(\vec y))$.
% \end{enumerate}
...
A particularly simple method to transform estimates of measurement data to estimates of quantum states is the method of linear inversion, which we are going to review now:
...
However, in the unconstrained setting, the ellipsoidal construction~\eqref{eq:ortho.confelip} is known to be admissible only for $m=\left\{ 1,2\right\}$~\cite{Joshi_1969_Admissibility}, while it is not admissible for $m\geq3$~\cite{Joshi_1967_Inadmissibility} due to Stein's phenomenon~\cite{Stein_1956_Inadmissibility}.
...
We are in position to provide the first result of this paper, falling into the category of geometry of quantum states:
...
% By Thm.~\ref{thm:ortho.ellipsoids}, the untruncated region $\CR$ can be easily characterized:
% It is an ellipsoid centered around $\estim\varrho$, where $\estim{\vec\varrho} = A^{-1} \estim{\vec y}$, and its characteristic properties such as diameter, volume, etc.\ can be readily expressed in terms of the $R_i$.
...
% After computing the ellipsoidal estimator $\CR$ for a fixed experimental outcome, we could have two situations: precisely, we s
% \begin{enumerate}
%   \item If $\CR\subset\States+d$, then $\CR$ and $\CR^\cap$ are identical and, therefore, have the same volume, diameter, etc.
%   \item If $\CR$ covers Hermitian matrices (necessarily the set of non-zero measure) beyond $\States$, then $\CR^\cap$ has strictly smaller volume than $\CR$.
%   Note that the diameter may be unaffected.
% \end{enumerate}
% It becomes evident that while computing the volume of $\CR^\cap$ one also provides an answer to the following question: \emph{%
%   Is the ellipsoid~\eqref{eq:ortho.ellipsoid} fully contained in the set $\States$ of positive semi-definite density matrices?
% }
% Our goal is to show that there is no computationally efficient algorithm deciding this problem.

More precisely, we are concerned with the question if for a fixed ellipsoid $\CR$ there is any reduction in size due to constraints in Eq.~\eqref{eq:ortho.truncated_ellipsoid} or if $\CR$ is fully contained in the set of psd states.
...
Let us start the outline of the proof of Thm.~\ref{thm:ortho.hard} with a simplified example, when the ellipsoid in question is a ball, i.e.\ when $R_{i}=R$ for all $i=1,\ldots,d^{2}-1$.
...
Now, in order to prove the computational intractability of Problem~\ref{prob:ortho.ellpos}, we use a reduction from the \emph{balanced sum} problem, which is known to be NP-complete.
...
% Looking from tomographic perspective, this result shows how complex the optimal error estimation happens to be, when one deals with quantum states instead of a bunch of unconstrained parameters.
% From the point of view of orthodox statistic it is thus fully legitimate not to aim for a general-purpose, efficient routine able to output an optimal confidence region and, as a consequence, allow for approximations.
% This conclusion, however, would not be practically meaningful if one could bypass the limitations of Theorem \ref{thm:ortho.hard} with the help of Bayesian tools.
% In the following section we will show that the intractability result has a universal flavor, and while looking for optimal Bayesian credibility regions one is back in Problem \ref{prob:ellpos}.
