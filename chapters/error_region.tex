...
Instead of proving that a given problem is infeasible to solve efficiently, we are comparing its computational complexity to other problems.
The goal is to show that it is at least as hard to solve as a large body of problems, which have withstood all attempts at solving them efficiently so far.
...
Indeed, by using  the reduction $f$, we can expand any TM deciding $L'$ to also decide $L$ with at most polynomial runtime overhead.
