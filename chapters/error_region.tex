...
Consequently, we should not only report the final result of an experiment, but we also should answer the question whether this result is statistically reliable or arose due to chance.
Of course, such problems do not only occur in the analysis of quantum experiments but in all empirical sciences such as psychology, demography, or medicine.
To answer said question, the field of statistical inference provides a vast number of methods such as hypothesis testing or \quotes{error bars}.
In this chapter, we investigate the problem of providing error bars for quantum state estimation~(QSE), i.e.\ the task of reconstructing the density matrix of a finite dimensional quantum system associated to a experimental preparation procedure from measurements.
...
However, what sets the task of QSE apart from inference in GLMs in general are the \emph{quantum mechanical shape constraints}:
...
Nevertheless, there are estimators for $\varrho$ that on one hand always yield a psd density matrix, and on the other hand, are well-understood theoretically with near-optimal performance and scalable to intermediate sized quantum experiments~\cite{Paris_2004_Quantum}.
