...
% Note that the risk in \cref{eq:intro.frequentist.risk} -- and therefore the performance of a given estimator -- still depends on the unknown true value $\theta_0$.
% Some definitions risks that are independent of $\theta_0$ are given in the following definition.
% \begin{definition}
  % \label{def:intro.frequentist.optimality_conditions}
  % \begin{itemize}
...
Note that the risk function~\eqref{eq:intro.frequentist.risk} for a given estimator still depends on the unknown true value.
...
The Gaussian approximation of the measurement statistics in \eqref{???} suggests that we use a Gaussian prior for $\varrho$:
\todo{Fill in reference!}
...
In order to incorporate constraints on $\varrho$ imposed by quantum mechanics, we chose a prior distribution that is concentrated on $\States$ and vanishes on its complement.
Since our statistical
