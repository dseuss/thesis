...
Any information we obtain about a quantum system through measurements is subject to statistical uncertainty due to the intrinsic randomness of quantum mechanics.
Consequently, we should not only report the final result of an experiment but also should answer the question whether this result is statistically reliable or simply due to chance.
Of course, such problems not only arises in the analysis of quantum experiments but in all kinds of empirical sciences such as psychology, demography, or medicine.
In order to answer this question, the field of statistical inference provides a vast number of methods such as hypothesis testing or \quotes{error bars}.
In this chapter, we are investigating the problem of providing error bars for quantum state estimation~(QSE), i.e.\ the task of reconstructing the density matrix of a finite dimensional quantum system associated to a experimental preparation procedure from measurements.

More precisely, we consider the task of non-adaptive QSE, where a fixed set of measurements specified by the choice of positive operator valued measure (POVM) is performed on independent copies of the system.
...
The well-established theory of GLMs then provides \quotes{good} estimators and error bars for $\varrho$~\cite{FIXME}.
However, what sets the task of QSE apart from standard GLMs are the \emph{quantum mechanical shape constraints}:
For $\varrho$ to be a valid quantum state, it has be a hermitian, positive semi-definite (psd) matrix with unit trace.
While the hermiticity and trace constraints are linear and easily satisfiable by means of a suitable parametrization, the psd constraint is non-linear and, hence, more problematic.
Nevertheless, estimators for $\varrho$ exist, which on one hand always yield a psd density matrix, and on the other hand, are well-understood theoretically with near-optimal performance and scalable to intermediate sized quantum experiments~\cite{FIXME}.
...
This motivates the research program this work is part of: \emph{Understand the general impact of positivity constraints on uncertainty quantification in QSE}.

% one question: is there an efficient algorithm that computes optimal error regions  for the QSE problem with especial emphasis on the psd constraints
% this work: provide evicdence that no such algorithmn can exist if well accepted conjectures in computational complexity hold
% more specifically, by considering models that render the unconstrained problem tractable, we show that computaional intractability arises due to constraints and not due to general difficulties of stats in high dimensions
...
However, note that the risk function~\eqref{eq:intro.frequentist.risk} for a given estimator still depends on the unknown true value.
...
\begin{definition}{\cite[Def. 12.17]{Wasserman_2013_All}}
  \label{def:intro.frequentist.admissability_point_estimators}
...
\[
  \label{eq:intro.frequentist.empirical_mean}
...
\[
  \label{eq:intro.frequentist.mle_derivation}
  \begin{split}
...
Now consider the multivariate generalization of the above problem, that is the task of estimating $\vec\theta_0 \in \Reals^d$ from a linear Gaussian model $\vec X \sim \Normal(\vec\theta_0, \1)$ with unit covariance matrix.
Suppose, we only have a single observation $\vec X$ -- that is $m=1$.
Since all the components $X_i$ are independent, one expects that $\bar{\vec X} = \vec X$ constitutes a \quotes{good estimator}.
Indeed, the same computation as in \cref{eq:intro.frequentist.mle_derivation} shows that the empirical mean is the MLE for this model as well.
...
\[
  \label{eq:intro.frequentist.stein_estimator}
...
Interestingly, \cref{eq:intro.frequentist.stein_estimator} shows that by taking into account all the components $X_i$, we can improve the MSE of the mean-estimator even if the $X_i$ are independent.
However, this only applies to the \emph{simultaneous} MSE error of all the components of $\vec \theta_0$.
The James-Stein estimator~\eqref{eq:intro.frequentist.stein_estimator} cannot be used to improve the MSE of a single component.
\Cref{eq:intro.frequentist.stein_estimator} can also be generalized to the case of more than one observation, i.e.\ $m > 1$.
Finally, note that the James-Stein estimator is also not admissible:
More elaborate shrinkage estimators are known to outperform~\eqref{eq:intro.frequentist.stein_estimator} w.r.t.\ the MSE.
...
Similar to point estimators, \cref{eq:intro.frequentist.coverage} does not uniquely determine a confidence region construction.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Estimation under quantum constraints}
\label{sec:error.qse}

% why tomography? -> other tasks
...
By shifting the center of the ellipsoid from the empirical mean $\hat{\vec y}$ to the Stein estimator~\eqref{eq:intro.frequentist.stein_estimator}, one can improve the coverage while keeping the volume constant~\cite{Joshi_1967_Inadmissibility}.
...
\todo{Explain the statistical model \& likelihood function}
\todo{Covariance matrix, parameter space.}
% Use Audenaart Scheel as a bluepritn
% Let us now turn to the , for a more thorough discussion see e.g.~\cite{Granade_2015_Practical,Audenaert_2009_Quantum}.
