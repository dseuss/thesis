...
\begin{itemize}
  \item An infinite tape, that is a bi-directional line of cells that can take the values from a finite set $\Gamma$ called the \emph{alphabet}.
    Besides the values 0 and 1, $\Gammma$ must contain a designated symbol $\blank$ called \quotes{blank}.
...
Furthermore, we assume that only a finite number of tape cells have value 0 or 1 -- these are referred to as the input.
All other cells have value $\blank$.
...
If the TM halts, the state of the output tape is taken as its output.

% computational problem: infinte collection of instances of varying size and the desired output
% to assess whether a given algorithm is efficient based on its runtime -- or more precisely the number of basic operatoins -- scaling as a function of input size for the worst case
% for this purpose -> turing machines as tractable model
% efficcient algorithm: runtime scales polynomially in n
% P definitoin
...
% but how to prove that computational problem is actually hard -> show there is no algorithm in poly time
% such nonexistence results exist only for computational models which are less poewrful than Turing model
% therefore, strategy: introduce certain class of problems, which are conjectured to be hard and show that problem under consideration is at least as hard
% -> NP -> reason for this is existence of hard problems
% showing hardness via reduction from... -> Def. reduction
% Def. NP hardness, completeness
% => computational intractble because otherwise solve a myriad of problems, which have evaded efficient solution for long time
% => can't find efficient algoithm  but neither can all these famous people
% question whether NP hard really do not have efficeint algorithm => P = NP question


% Example -> input, output -> decission problem
% partition problem one of six original NP hard problems, only one dealing with numbers -> starting point for many hardness proofs involving numbers
% see Garey Johnson for proof, here slightly different formulation
