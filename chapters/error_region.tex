...
We now formalize the notion of runtime.
Denote by
\[
...
\begin{definition}{\cite[Def.\ 1.3]{Arora_2009_Computational}}
  ...
  Let $f\colon \BitStrings \to \BitStrings$ and $T\colon \Reals \to \Reals$ be some functions.
  Furthermore, let $M$ be a Turing machine.
  We say $M$ \emph{computes} $f$ in $T(n)$-time if for every input $x \in \BitStrings$, whenever $M$ is initialized with input $x$ , then it halts with $f(x)$ written on its output tape in at most $T(\abs{x})$ steps.
\end{definition}
