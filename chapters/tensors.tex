...
\begin{figure*}
  \begin{subfigure}{\columnwidth}
    \includegraphics[width=0.32\textwidth,trim={97 240 120 217}]{fig/tmp/inset_X_N_3}
    \includegraphics[width=0.32\textwidth,trim={97 240 120 217}]{fig/tmp/inset_X_N_6}
    \includegraphics[width=0.32\textwidth,trim={97 240 120 217}]{fig/tmp/inset_X_N_13_new}
    \caption{%
      CDF $\Prob(X \le t)$ and approximations for $X = \prod_{i=1}^N g_i$
    }
  \end{subfigure}
  \begin{subfigure}{\columnwidth}
    \includegraphics[width=0.32\textwidth,trim={97 240 120 217}]{fig/tmp/inset_Y_N_3_new}
    \includegraphics[width=0.32\textwidth,trim={97 240 120 217}]{fig/tmp/inset_Y_N_6_new}
    \includegraphics[width=0.32\textwidth,trim={97 240 120 217}]{fig/tmp/inset_Y_N_13_new}
    \caption{%
      CDF $\Prob(Y \le t)$ and approximations for $Y = \prod_{i=1}^N g_i^2$
    }
  \end{subfigure}
  \begin{subfigure}{\columnwidth}
    \includegraphics[width=0.32\textwidth,trim={97 240 120 217}]{fig/tmp/inset_Z_N_3}
    \includegraphics[width=0.32\textwidth,trim={97 240 120 217}]{fig/tmp/inset_Z_N_6_new}
    \includegraphics[width=0.32\textwidth,trim={97 240 120 217}]{fig/tmp/inset_Z_N_13_new}
    \caption{%
      CDF $\Prob(Z \le t)$ and approximations for $X = \prod_{i=1}^N g_i$
    }
  \end{subfigure}
  \caption{%
    \label{fig:gaussians.cdfs}%
    The plots of the  CDFs (red lines) of the random variables $X$, $Y$, and $Z$ with their power-log series \eqref{eq:gaussians.series}
    truncated at $k=0$ (green lines) and $k=1$ (blue lines),
    Each inset shows the approximation error of the truncations on a logarithmic scale.
...
The first order approximation behaves similarly, but the approximation error remains small enough so that it cannot be distinguished from the true cdf on the linear plots.
\todo{Really?}
Therefore, we provide insets showing the logarithmic approximation error.
There, we see that the first-order approximation error remains below $10^{-5}$ for all shown.
...
In conclusion, the numerical simulations depicted in \cref{fig:gaussians.cdfs} shows that the power-log series expansion of the cdfs of the random variables $X$, $Y$, and $Z$ provides excellent approximation even for very low truncation order.
...
In the rest of this section, we investigate those properties through numerical simulations.
