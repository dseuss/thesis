...
Similarly, a \emph{local purification matrix product state} (PMPS) is a matrix product state with two legs per site as depicted in \cref{fig:mps.pmps} such that
\[
  \underbrace{%
    \begin{tikzpicture}[baseline=-.5ex]
      \mpa{4}{{N=1, S=1, tensor_width=0.5, tensor_height=0.5, len_horizontal_legs=0.1}}
...
Another use of the MPS tensor format in machine learning was pioneered in~\cite{Stoudenmire}.
...
% MPS: efficient tesnor description, structure well suited for many practical applications
% problem in practice: recover tensor X from measurable quantities -> how many measurements? algorithm to reconstruct?
% due to curse of dimensionality -> efficiency and low sample complexity important in tensor case.
%
% example: QSE for MPS -> cite
% measurements: informationally complete k-local with k << N => sample complexity ~ N; 
% computationally efficient algorithm in MPS rep; convergence proof only for ineffient version

% similar problem: phaselift chapter -> low-rank matrix recovery -> structure leads to reduced sample complexity
...
% naive aproach -> d^2 such measurements
% put generaliztt of THM -> 
...
% computationally efficient -> SDP or other algorithmk
...
% result for MPS of rank k and order 2 -> generalization to arbitary orderes
% The goal is to reconstruct $X$ from $m$ linear measurements of the form simlar to above, with measuemetn tensors $A$


% existing work: canonical and tucker decompositoin -> still exponential sampling complexity
% zeljka: projected gradient method for MPS, poly sample complexity BUT: fully gaussian measurements
% cannot be computed efficeintly since measurements ~ exponential
% similar to matrix case; usually first step: rank 1 measuement model with gaussian local tensors
% 
