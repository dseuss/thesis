...
\end{align}
provided the (generally non-square) matrices $L\ind{l}$ and $R\ind{l}$ satisfy $L\ind{l} R\ind{l} = \1$ as then
\begin{equation}
  \begin{tikzpicture}[baseline=-.5ex]
  ...
  \end{tikzpicture}
\end{equation}
As shown in~\cite[Thm. 2]{Perez-Garcia}, these local transformations on the virtual degrees of freedom are the only possible gauge transformations leaving $X$ invariant.
