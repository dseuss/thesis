...
\begin{theorem}%
  ...
  \[
    \sqrt{1 - \braket{x_i, y_i^1}} \le \theta^{2^{i - 1}} \delI.
...
Therefore, an alternating left-right-left sweep may be more beneficial for improving the global error of the reconstruction.
Such a strategy also is widely used in related algorithms such as DMRG.
...
% also: insensitive to sign -> might reconstruct -x_i (or x_i up to complex phase for complex tensors)
% on the one hand, this is due to gauge degree of freedom
% global sign can be fixed by a single additiional measurement
...
\]
for $i \neq j$.
The ability to reconstruct all local tensors only up to sign implies that we are able to reconstruct $X$ only up to a global sign using ALS.
