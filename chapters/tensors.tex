...
\[
  y\ind{l} = \braket{A\ind{l}, X}, \ldots l=1,\ldots,M,
\]
...
Instead, we iteratively optimize the empirical error over a single local tensor at a time Similarly to the simplest version of DMRG.
...
\begin{algorithm}[H]
  ...
  \For{$h \gets 1$ \KwTo $H$}{
    ...
    \For{$n \gets 1$ \KwTo $N$}{
      ...
      $Y_n \gets \mathtt{left\_normalize}(\hat Y)$
    }
  }
...
% ALS
% alternating algorithms very naaaatural for MPS: compression, DMRG, etc.
% local convergence for gennnnnearl minimiization well studied in Rohwedder/Ushmajev
% proposed for empirical error minimization e.g. in ... and especially for reconstruction of tensors from few measurements in ...
% as discussed in intro: no global convergence proof for poly nr. of measurements
%
% why this instead of iht -> depends on tensor RIP -> show in appendix that thsi is not fulfilled
...
Local convergence of these alternating algorithms has been proved for a large class of problems~\cite{Rohwedder}.
...
Furthermore, the problem of proving that the minimizer of the empirical $\ell_2$ error~\eqref{eq:als.empirical_l2} with given rank is equal to $X$, i.e.\ that the given measurements suffice to identify $X$.
