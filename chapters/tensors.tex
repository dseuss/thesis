...
\[
  \sum_l \left( b\ind{l} - \braket{A\ind{l}, Y} \right)^2.
...
\begin{algorithm}[H]
  ...
  \Input{%
    ...
    Measurement tensors $A\ind{l}$ and measurement outcomes $b\ind{l}$ with $i = 1, \ldots, H \times N \times m$ \newline
    Divide $(A\ind{l}, y\ind{l})_l$ into $HN$ subsets of size $m$, which are denoted by ${\left(A\ind{h,n;l}, b\ind{h,n;l}\right)}_l$ ($h \in [H], n \in [N]$)
  }
  \BlankLine
...
For the remainder of this section, we investigate the distribution and scaling w.r.t.\ the parameters $N$ and $d$ of the crucial constant $\frac{\delB}{\delG}$ introduced in \cref{sub:als.ana}.
  ...
  \For{$h \gets 1$ \KwTo $H$}{
    ...
    \For{$n \gets 1$ \KwTo $N$}{
      \For{$l \gets 1$ \KwTo $m$} {
        ...
        $B\ind{l} \gets \mathtt{contract}(A\ind{l}, Y_{[N] \backslash n})$ \label{line:als_practical.partial_contraction}
      }
...
\begin{figure*}
  ...
  \caption{%
    ...
    Recovery error of the ALS algorithm as
  }
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
