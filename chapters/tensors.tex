...
\begin{algorithm}[H]
  ...
  \Input{%
    ...
    Measurement tensors $A\ind{l}$ and measurement outcomes $b\ind{l}$ with $i = 1, \ldots, H \times N \times m$ divided into $HN$ batches of size $m$, which are denoted by ${\left(A\ind{h,n;l}, b\ind{h,n;l}\right)}_l$ ($h \in [H], n \in [N]$)
  }
  \BlankLine
...
In the first lemma, we relate the \quotes{global} distance of two tensors, which is measured in Frobenius norm, to the distances of the local tensors measured by~\eqref{eq:ana.infidelity}.
\begin{lemma}%
...
In contrast to previous work, which either requires an exponentially large number of measurements or incompressible Gaussian measurement tensors, our work investigates recovery using Gaussian product measurements.
Therefore, the proposed scheme aims no only to be sample efficient, but also computationally efficient.

The main result of this chapter is the sufficient condition for reconstruction from rank-one measurements via ALS in \cref{eq:ana.local_convergence}.
...
However, note that the ALS algorithm does not unnecessarily improve the error of the full tensor in each micro-iteration:
...
Second, the compression of $\tilde X_\mathrm{init}$ to the appropriate rank does not invalidate this condition.
...
This is not surprising as the compression regularizes the estimate -- this is exactly the effect that allows for recovering $X$ from fewer than $d^N$ measurements.
