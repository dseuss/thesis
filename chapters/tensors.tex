...
For a any, $D \times D$ mixed state $\rho$, a purification is a pure state $\ket{\psi} \in \Complex^{D \times D}$ such that~\cite{Nielsen_2010_Quantum}
\[
  \rho = \tr_2 \ket{\psi}\bra{\psi}.
\]
Here, the partial trace is taken over the second auxiliary Hilbert space.
Similarly, for a MPDO $\rho$, a \emph{local purification matrix product state} (PMPS) is a matrix product state with two legs per site as depicted in \cref{fig:mps.pmps} such that
\[
  \underbrace{%
    \begin{tikzpicture}[baseline=-.5ex]
      \mpa{4}{{N=1, S=1, tensor_width=0.5, tensor_height=0.5, len_horizontal_legs=0.1}}
...
But this advantages comes at a price:
\todo{Cuevas Paper}
