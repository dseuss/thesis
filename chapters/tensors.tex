...
\begin{figure*}
  ...
  \caption{%
    \label{fig:mps.mpo}%
    An MPO with open boundary conditions.
  }
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure*}
  ...
  \caption{%
    \label{fig:mps.pmps}%
    An PMPS with open boundary condition.
  }
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure*}
  ...
  \caption{%
    Distribution function of $\log Y = \log X^2 = \log \left(\prod_{i=1}^n g_i^2 \right)$ with $Y$ given by \cref{eq:gaussian.X} for different numbers of factors $n$.
...
Recall from \cref{sub:als.ana} that one of the conditions needed for successful recovery via ALS is a lower bound on $\lambda_\mathrm{min}(B_j(v))$ with $B_j(v)$ from \cref{eq:als.gaussian.B}.
...
\begin{figure*}
  ...
  \caption{%
    ...
    \Cref{fig:gaussians.cdf.f} shows the CDFs ($k=\infty$) of the random variables $X$, $Y$, and $Z$ with their power-log series~\eqref{eq:X_expansion}--\eqref{eq:Z_expansion}, respectively,  truncated at different orders $k$.
...
\begin{figure*}
  ...
  \caption{%
    ...
    Empirical distribution of $\frac{Z_\mathrm{B}}{Z_\mathrm{G}}$ for Gaussian product measurements.
    ...
    For each combination of the number of sites $N$ and local dimension $d$, we choose the number of measurements $m = CNd$.
...
\subsection{Numerical reconstruction}%
...
\begin{figure*}
  ...
  \caption{%
    ...
    Recovery error of ALS (Algorithm~\ref{alg:als_practical}) as a function of the epoch $h$.
    All pictures show the recovery of a fixed randomly chosen tensor $X \in \Reals^{d^N}$ for $d = 4$ and $N = 8$.
    Each line corresponds to a different set of Gaussian product measurements.
