...
The idea to recover $X$ is simply to find the tensor $Y$ of desired rank that minimizes the empirical $\ell_2$ error $\sqrt{\sum_l \left( y\ind{l} - \braket{A\ind{l}, Y} \right)^2}$.
...
Finally, note that the analysis of the algorithm below requires that the samples at each update steps are independent.
...
% ALS
%
% compared to IHT: faster; no RIP required
