...
\]
Finally, we assume that the rank-1 measurement tensors defined in \cref{eq:als.rank1_measurements} satisfy the following conditions:
\begin{enumerate}
...
Assume that the initialization satisfies
\[
  \braket{x_i, y_i^0}^2 \ge \sqrt{1 - \delta^2} \quad 1 = 2, \ldots, N.
\]
This implies
\begin{align}
  \kappa_1^0
  &= \frac\delB\delG \prod_{i \neq 1} \Abs{\braket{x_i, y_i^0}} - \prod_{i \neq 1} \sqrt{1 - {\braket{x_i, y_i^0}}^2} \\
  &\ge \frac\delB\delG \left( 1 - \delta^2 \right)^{\frac{N - 1}{2}} - \delta^{N-1} \\
  &=: \kappa_\delta.
\end{align}
Therefore, assuming $\kappa_\delta > 0$, we have with \cref{lem:ana.local_error_bound}
\begin{align}
  1 - \braket{x_1, y_1^0}^2
  &\le \frac{1}{\kappa_\delta^2} \, \prod_{i \neq 1} (1 - \braket{x_i, y_i^0}^2)  \\
  &\le \frac{\delta^{2(N - 2)}}{\kappa_\delta^2} \delta^2. \\
  &= \frac{1}{\tilde\kappa_\delta^2} \, \delta^2.
  \label{eq:ana.contraction}
\end{align}
Assuming that the constant
\[
  \tilde\kappa_\delta = \frac{\kappa_\delta}{\delta^{N - 2}}
\]
is larger than one, \cref{eq:ana.contraction} implies that the upper bound on the overlap of the first local tensors decreases.

We can now iterate this for the next step:
\begin{align}
  1 - \braket{x_2, y_2^0}^2
  &\le \frac{1}{\kappa_\delta^2} \, (1 - \Braket{x_1, y_1^1}^2)\prod_{i > 2} (1 - \Braket{x_i, y_i^0}^2) \\
  &\le \frac{1}{\kappa_\delta^2} \, \frac{\delta^2}{\tilde\kappa_\delta^2} \, \delta^{2(N - 2)} \\
  &= \frac{1}{\tilde\kappa_\delta^4} \, \delta^2. \\
  \label{eq:ana.contraction}
\end{align}

The condition $\tilde\kappa_\delta > 1$ translates to
\[
  \left(\frac\delB\delG\right)^2 > \left( \frac{\delta^2}{1 - \delta^2} \right)^{N - 1} \, \left( \frac{1 + \delta}{\delta} \right)^2.
\]
