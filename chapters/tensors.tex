  ...
  \begin{enumerate}
    ...
    \item\label{lbl:conditions.B}\textbf{Concentration of the $B_j$ operators}:
      ...
      \[
        ...
        B_j(v_1, \ldots, v_N) = \frac{1}{m} \sum_{l=1}^m \left( \prod_{i\neq j} \Braket{a\ind{l}_i, v_i} \right)^2 \Ketbra{a\ind{l}}.
      \]
      Then, with high probability the smallest eigenvalue of $B_j(v_1, \ldots, v_N)$ should satisfy
      \[
        \label{eq:ana.conditions.lmin}
        \lambda_\mathrm{min}\left(B_j(v_1, \ldots, v_N)\right) \ge 1 - \delta.
      \]
    \item\label{lbl:conditions.G}\textbf{Concentration of the $G_j$ operators}:
      ...
      \[
        ...
        G_j(v_1, v_1^\perp, \ldots, v_N^\perp) = \frac{1}{m} \sum_{l=1}^m \left( \prod_{i\neq j} \Braket{a\ind{l}_i, v_i} \Braket{a\ind{l}_i, v_i^\perp} \right) \Ketbra{a\ind{l}}.
      \]
      Then, with high probability the largest eigenvalue of $G_j(v_1, v_1^\perp, \ldots, v_N^\perp)$ should satisfy
      \[
        \label{eq:ana.conditions.lmax}
        \lambda_\mathrm{max}\left(G_j(v_1, v_1^\perp, \ldots, v_N^\perp)\right) \le \delta
      \]
  \end{enumerate}
...
\begin{lemma}%
  ...
  \begin{align}
    ...
    \tilde G_1 &= \frac{1}{m} \sum_l {\left( \prod_{i \neq 1} \Braket{a\ind{l}_i, \tilde x_i^\perp} \Braket{a\ind{l}_i,y_i} \right)}\Ketbra{a\ind{l}_1} \\
    \tilde x_i^\perp &= \left(\ketbra{y_i} - \1 \right) x_i.
  \end{align}
\end{lemma}
Note that \cref{thm:ana.conditions} assumes concentration properties for operators of the same type (up to normalization) as $\tilde B_1$ and $\tilde G_1$.
\begin{proof}
  ...
  \[
    ...
    \underbrace{ \frac{1}{m} \left[ \sum_l \left( \prod_{i \neq 1} \Braket{a\ind{l}_i, x_i} \Braket{a\ind{l}_i, y_i} \right) \, \Ketbra{a\ind{l}_1} \right]}_{=: \tilde O_1} \Ket{x_i}.
  \]
...
\end{lemma}
\begin{proof}
  First note that $\tilde B_1 = B_1(y_2, \ldots, y_N)$.
  ...
  \]
  Furthermore, $\tilde G_1 = G_1(y_2, \tilde x_2^\perp, \ldots, \tilde x_N^\perp)$, and hence, normalization of the $\tilde x_i^\perp$ yields
  \[
    \label{eq:ana.error_term_bound.G}
    \begin{split}
      \Opnorm{\tilde G_1}
      &\le \delta \prod_{i \neq 1} \ltwonorm{\tilde x_i^\perp} \\
      &\le \delta \prod_{i \neq 1} \sqrt{1 - {\braket{x_i, y_i}}^2}.
    \end{split}
  \]
  ...
  Similarly, we obtain for any $x_1^\perp$ with $\ltwonorm{x_1^\perp} = 1$ and $\braket{x_1, x_1^\perp} = 0$
  \begin{align}
...
\begin{lemma}%
  ...
  If $\prod_i \alpha_i^2 \ge 1 - \epsilon^2$ holds, then
  \[
...
\begin{proof}
  ...
  \]
  $g$ and $\Omega$ fulfill the conditions of Proposition~2.1 in \cite{Taylor}.
  Therefore,
  \[
  ...
  By substituting $\eta_i = \alpha_i^2$, \cref{eq:ana.kappa_bound.g_is_positive} and the assumption $\prod_i \alpha_i^2 \ge 1 - \epsilon^2$ gives
  \[
    \prod_i \left( 1 - \alpha_i^2 \right) \le 1 - \prod_i \alpha_i^2 \le \epsilon^2.
  \]
  Therefore, we obtain the claim of this Lemma via
  \begin{align}
